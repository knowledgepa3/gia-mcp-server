...
import { getMetricsStore, getGovernanceEventsStore, getBaselines } from './tools/value-metrics.js';
...
setInterval(() => {
  ...
  // For production scale, replace with a TTL-based Map or Redis.
 ...
 */
function verifyWebAuthnProofToken(token: string, expectedGateId?: string, expectedAction?: string): VerifiedProof | null {
  ...
  try {
    ...
    // 3. JTI replay protection — single-use enforcement
    if (!payload.jti) return null;
    if (usedJTIs.has(payload.jti)) return null; // REPLAY DETECTED
    usedJTIs.add(payload.jti);

    // Schedule JTI cleanup after TTL
    setTimeout(() => usedJTIs.delete(payload!.jti), JTI_TTL_MS).unref();
...
async function startHttpServer(): Promise<void> {
  ...
  async function handleDashboardAPI(
    ...
    // All /api/gia/* endpoints are dashboard REST — no auth required
    // (access controlled by Nginx, same as /api/* to Express)
      ...
      try {
        ...
        if (bodyText) {
          ...
          if (body.proofToken) {
            ...
            const verified = verifyWebAuthnProofToken(body.proofToken, gateId, 'approve');
            if (verified) {
    ...
    if (path.match(/^\/api\/gia\/gates\/gate-[^/]+\/reject$/) && req.method === 'POST') {
      ...
      try {
        ...
        if (bodyText) {
          ...
          if (body.rationale) rationale = String(body.rationale);
          if (body.proofToken) {
            const verified = verifyWebAuthnProofToken(body.proofToken, gateId, 'reject');
            if (verified) {
    ...
    // GET /api/gia/report — impact report with real value metrics
    if (path === '/api/gia/report' && req.method === 'GET') {
      ...
      // Value metrics from in-memory store (real — populated by record_value_metric MCP tool)
    ...
    // GET /api/gia/compliance?framework=X — compliance framework coverage
    // Data source: static COMPLIANCE_MAPPINGS (16 real control mappings across 4 frameworks)
    if (path === '/api/gia/compliance' && req.method === 'GET') {
      ...
      if (fw === 'ALL') {
        ...
        sendJSON(res, 200, { mappings: grouped });
      } else {
  ...
  async function getOrCreateTenantSession(tenantId: string): Promise<TenantSession> {
    ...
    const { server, engine } = await createGIAServer('tenant');
    ...
    if (parsedUrl.pathname === '/.well-known/mcp/server-card.json') {
      sendJSON(res, 200, {
        ...
        authentication: {
          required: true,
          schemes: [
            { type: 'header', name: 'x-api-key', description: 'GIA API key' },
